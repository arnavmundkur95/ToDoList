...
function sort() {
  ...
  // Assign divs priority based on color
  for (const it of items) {
    var temp = getComputedStyle(it).getPropertyValue("background-color");

    var r = temp.split(",")[0].match(/(\d+)/)[0];
    var g = temp.split(",")[1].match(/(\d+)/)[0];
    var b = temp.split(",")[2].match(/(\d+)/)[0];
    var rgb = "rgba(" + r + "," + g + "," + b + "," + opac + ")";

    // console.log(it.getAttribute('priority'));
    for (const colo of colorPriority.keys()) {
      if (rgb == colo) {
        it.setAttribute("priority", colorPriority.get(rgb));
      }
    }
  }

  // Get list of divs and sort by priority
  // Converting object to list code:
  // https://stackoverflow.com/questions/38824349/how-to-convert-an-object-to-an-array-of-key-value-pairs-in-javascript
  var listOfItems = Object.keys(items).map(function(key) {
    return [Number(key), items[key]];
  });

  listOfItems.sort((a, b) =>
    a[1].getAttribute("priority") < b[1].getAttribute("priority") ? 1 : -1
  );

  // console.log(listOfItems[0][1].innerText);

  // for(var i = 0; i < listOfItems.length-1; i++){
  //   listOfItems[i+1].insertAfter(listOfItems[i]);
  // }
